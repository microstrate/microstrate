...
async function main() {
  ...
  await copyFile('list.json', path.join(INTEGRATIONS_DIR, 'list.json'));

  // 3. Read current directory contents
  const entries = await readdir('.', { withFileTypes: true });

  // Filter to only directories except deployment itself
  const folders = entries.filter(
    (e) => e.isDirectory() && e.name !== 'deployment'
  );

  for (const folder of folders) {
    const folderName = folder.name;
    const folderPath = `./${folderName}`;
    const targetFolder = path.join(INTEGRATIONS_DIR, folderName);
    ...
    try {
      ...
      console.error(`Failed to read or parse JSON files in folder ${folderName}:`, err);
